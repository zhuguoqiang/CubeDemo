...
var CubeWhiteboardDelegate = Class({
	...
	didRevoked: function(wb) {
		if (null != this.engine.wbListener) {
			this.engine.wbListener.onRevoked(wb);
		}
	},
...
var PencilEntity = Class(GraphicsEntity, {
	...
	draw: function() {
		...
		this.lastY = this.lineY[this.lineY.length - 1];
...
var CubeWhiteboard = Class({
	...
	resize: function(width, height) {
		...
		if (this.wrapWidth > 0 && this.wrapHeight > 0) {
			var dx = width - this.wrapWidth;
			var dy = height - this.wrapHeight;

			this.paper.setSize(this.wrapWidth, this.wrapHeight);
		...
		if (null != this.curEntity) {
			this.curEntity.onResize(this.wrapWidth > 0 ? this.wrapWidth : width, this.wrapHeight > 0 ? this.wrapHeight : height);
		}
	...
	load: function(width, height) {
		...
		this.background.style.backgroundColor = "#efefef";
	 ...
	 * 矫正画布大小跟随容器大小。
	 */
	adjustSize: function(wrapWidth, wrapHeight) {
		if (wrapWidth !== 'undefined' && wrapHeight !== 'undefined') {
	...
	shareWith: function(list) {
		...
		if (null != this.sharedList) {
			if (this.sharedList.length != list.length) {
				...
				if (null != this.sharedList && count == this.sharedList.length) {
					// 分享列表没有变化，返回
					return true;
				}
			}
		...
		dialect.appendParam('data', data);
		return nucleus.talkService.talk(_WB_CELLET, dialect);
	},
	...
	onDialogue: function(action, dialect) {
		...
		else if (action == CubeConst.ActionRevokeAck) {
			var state = dialect.getParam("state");
			if (state.code == 200) {
				Logger.i('CubeWhiteboard', 'Revoked ok.');

				// 回调委派
				this.delegate.didRevoked(this);
			}
			else {
				Logger.e('CubeWhiteboard', 'Revoked failed.');

				// 回调委派
				this.delegate.didFailed(this, state.code);
			}
		}
		else if (action == CubeConst.ActionRevoke) {
			if (null != this.sharedList) {
				this.revokeSharing();
			}
		}
	...
	onSharingReady: function(file) {
		...
		slide.setOffline(this.readMode);
...
var CubeViewport = Class({
	...
	zoom: function(ratio) {
		var vbw = this.wb.width;
		var vbh = this.wb.height;
		...
		if (ratio < 1.0) {
			...
			vbw = vbw + dx;
			vbh = vbh + dy;
			this.posX -= parseInt(dx * 0.5);
			this.posY -= parseInt(dy * 0.5);
		}
		else if (ratio > 1.0) {
			dx = (ratio - 1.0) * vbw;
			dy = (ratio - 1.0) * vbh;
		...
		this.wb.paper.setViewBox(this.posX, this.posY, vbw, vbh, false);
	},
	...
	fireDown: function(event) {
		...
		var pos = (event.touches === undefined) ? utils.getMousePos(event) : utils.getTouchPos(event);
		this.tempX = pos.x;
		this.tempY = pos.y;
	},
	...
	fireMove: function(event) {
		...
		this.posY += parseInt(dy);
		if (this.viewW <= 0) {
			this.viewW = this.wb.width;
		}
		if (this.viewH <= 0) {
			this.viewH = this.wb.height;
		}
		this.wb.paper.setViewBox(this.posX, this.posY, this.viewW, this.viewH, false);
